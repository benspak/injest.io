...
export default function SentInboxPage() {
  ...
  const isSentEmail = (item: Item): boolean => {
    const source = (item.source || '').toLowerCase();
    return item.type === 'email' && source.startsWith('send_workflow:outbound');
  };
  ...
  const loadItems = useCallback(
    async (currentOffset: number = 0, reset: boolean = false) => {
      try {
        ...
        const sentEmails = fetched.filter(isSentEmail);
        ...
        if (reset) {
          ...
          setItems((prev) => {
            const combined = [...prev, ...sentEmails];
  ...
  const handleSearchResultsChange = useCallback((results: SearchResult[]) => {
    // Only show sent emails in search mode
    const filtered = results.filter(
      (result) => result.entityType === 'item' && result.item && isSentEmail(result.item),
    );
      ...
      <header className="bg-white border-b">
        <div className="container mx-auto px-3 sm:px-4 md:px-6 py-4 flex justify-between items-center max-w-full">
          <div className="flex flex-col gap-1">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Outbox</h1>
            <p className="text-xs sm:text-sm text-gray-500">
              Outbound emails sent via the Send workflow.
            </p>
          </div>
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-xs sm:p-6 min-h-[200px]">
          {searchQuery.length >= 2 ? (
            ...
            <div className="py-8 text-center text-muted-foreground">Loading sent emails…</div>
          ) : (
            <>
              ...
              {hasMore && (
                <div ref={loadMoreSentinelRef} className="py-4 text-center">
                  {loadingMore && (
                    <p className="text-sm text-muted-foreground">Loading more sent emails...</p>
                  )}
                </div>
              )}
              {!hasMore && items.length > 0 && (
                <div className="py-4 text-center">
                  <p className="text-sm text-muted-foreground">All sent emails loaded</p>
                </div>
              )}
              {!hasMore && items.length === 0 && !loading && (
                <div className="py-8 text-center text-muted-foreground">
                  No sent emails yet. Use the Send page to compose your first email.
                </div>
              )}
            </>
          )}
