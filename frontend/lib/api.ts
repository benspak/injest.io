...
class ApiClient {
  ...
  // Custom Domains
  async getCustomDomains(): Promise<{
    domains: Array<{
      id: string;
      user_id: string;
      domain: string;
  ...
  async createCustomDomain(domain: string): Promise<{
    domain: {
      id: string;
      user_id: string;
      domain: string;
  ...
  async verifyCustomDomain(id: string): Promise<{
    domain: {
      id: string;
      user_id: string;
      domain: string;
    ...
    return this.request<{
      ...
      verified: boolean;
      message: string;
    }>(`/api/custom-domains/${id}/verify`, {
      method: 'POST',
    });
  }

  async updateCustomDomain(id: string, updates: { is_active?: boolean }): Promise<{
    domain: {
      id: string;
      user_id: string;
      domain: string;
  ...
  async deleteCustomDomain(id: string): Promise<{ message: string }> {
    return this.request<{ message: string }>(`/api/custom-domains/${id}`, {
      method: 'DELETE',
    });
  }
