 ...
 */
router.get('/messages/:workspaceId/:channelId/:messageTs/context', authMiddleware, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        const { workspaceId, channelId, messageTs } = req.params;
        const context = await slackSearchService.getMessageContext(req.user.id, workspaceId, channelId, messageTs);
        res.json(context);
    }
    catch (error) {
