...
function sanitizeItem(result) {
    ...
    return {
        ...
        createdAt: item.created_at ? toIsoString(item.created_at) : null,
        updatedAt: item.updated_at ? toIsoString(item.updated_at) : null,
    };
}
function buildContactContext(contacts) {
    return contacts.map((contact) => ({
        ...
        phone: contact.phone,
        company: extractCompanyFromEmail(contact.email),
        lastInteraction: contact.metadata && typeof contact.metadata === 'object' && contact.metadata !== null
            ? contact.metadata.lastInteraction ?? undefined
            : undefined,
    }));
}
...
router.post('/plan', async (req, res) => {
    try {
        ...
        analysis.keyFacts.forEach((fact) => {
            const tokens = fact.split(/\s+/).filter((token) => token.length > 3);
            tokens.slice(0, 2).forEach((token) => keywords.add(token));
        });
        ...
        const searchQuery = analysis.searchQuery && analysis.searchQuery.trim().length > 0
...
router.post('/execute', async (req, res) => {
    try {
        ...
        const platforms = Array.isArray(platformsInput)
            ? platformsInput.filter((p) => typeof p === 'string' && (p === 'email' || p === 'xcom'))
        ...
        const toEmailInput = Array.isArray(toEmailInputRaw)
            ? toEmailInputRaw.filter((value) => typeof value === 'string' && value.trim().length > 0).map((email) => email.trim())
            : typeof toEmailInputRaw === 'string'
                ? toEmailInputRaw.trim()
                : '';
        const cc = Array.isArray(req.body?.cc)
            ? req.body.cc.filter((value) => typeof value === 'string' && value.trim().length > 0)
            : [];
        const bcc = Array.isArray(req.body?.bcc)
            ? req.body.bcc.filter((value) => typeof value === 'string' && value.trim().length > 0)
            : [];
        const replyTo = typeof req.body?.replyTo === 'string' && req.body.replyTo.trim().length > 0
        ...
        const attachmentsInput = Array.isArray(req.body?.attachments)
            ? req.body.attachments
                ...
                const attachmentFilename = typeof entry?.attachmentFilename === 'string' ? entry.attachmentFilename : null;
        ...
        if (platforms.includes('email')) {
            for (const attachmentRequest of attachmentsInput) {
                ...
                let match = attachmentRequest.attachmentFilename
                    ? itemAttachments.find((att) => att.filename === attachmentRequest.attachmentFilename ||
        ...
        if (platforms.includes('email') && recipientEmails.length > 0) {
            try {
                ...
                indexingService.indexItem(savedItem).catch((error) => {
