...
import { getXComServiceForUser } from '../services/xcom.js';
...
router.get('/callback', async (req, res) => {
    try {
        ...
        if (error) {
            return res.redirect(`${FRONTEND_URL}/dashboard?xcom_error=${encodeURIComponent(error)}`);
        }
        if (!code || !state) {
        ...
        const oauthService = getXComOAuthService();
        const stateData = oauthService.decodeState(state);
        if (!stateData) {
...
router.post('/post', authMiddleware, upload.single('image'), async (req, res) => {
    try {
        ...
        const xcomService = await getXComServiceForUser(req.user.id, userTokens, async (refreshedTokens) => {
