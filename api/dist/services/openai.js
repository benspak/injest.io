...
export class OpenAIService {
    client;
    EMBEDDING_MODEL;
    SIMPLE_TASK_MODEL = 'gpt-3.5-turbo'; // For simple tasks (10-30x cheaper than GPT-4)
    ADVANCED_TASK_MODEL = 'gpt-4-turbo-preview'; // For complex tasks that need GPT-4
    VISION_MODEL = 'gpt-4o'; // For vision tasks (supports image analysis)
    constructor() {
     ...
     */
    async executeWithRetry(apiCall, maxRetries = 5, baseDelay = 2000) {
        let lastError;
            ...
            try {
                ...
                if (isRateLimit) {
                    ...
                    if (attempt < maxRetries - 1) {
                        ...
                        console.warn(`[OpenAI] Rate limit hit (429), retrying in ${delaySeconds}s (attempt ${attempt + 1}/${maxRetries}). ` +
...
Content: ${contentPreview}`;
     ...
     */
    async generateImageDescriptionAndTitle(imagePath, filename) {
        try {
            ...
            const prompt = `Analyze this image and provide a title and description. Respond with JSON only:
{
  ...
  "description": "1-2 sentence description of what's in the image, max 200 chars"
     ...
     */
    async generateEmailSummary(emailBody) {
...
Email: ${contentPreview}`;
        try {
            ...
            if (Array.isArray(bullets) && bullets.length === 3) {
                return bullets.filter((bullet) => typeof bullet === 'string' && bullet.trim().length > 0);
            }
            return [];
        }
        catch (error) {
    ...
    async analyzeSendPrompt(prompt) {
        ...
        try {
            ...
            return {
                ...
                recommendedContactId: typeof parsed.recommendedContactId === 'string' && parsed.recommendedContactId
                    ...
                    : null,
                recommendedContactEmail: typeof parsed.recommendedContactEmail === 'string' && parsed.recommendedContactEmail
                    ? parsed.recommendedContactEmail
                    : null,
                contactReason: typeof parsed.contactReason === 'string' && parsed.contactReason.trim()
                    ? parsed.contactReason.trim()
                    : null,
                attachments: Array.isArray(parsed.attachments)
                    ? parsed.attachments
                        .filter((att) => typeof att === 'object' &&
                        att !== null &&
                        typeof att.itemId === 'string' &&
                        att.itemId.trim().length > 0)
                        .map((att) => ({
                        itemId: att.itemId,
                        attachmentFilename: typeof att.attachmentFilename === 'string' && att.attachmentFilename.trim().length > 0
                ...
                followUpTasks: Array.isArray(parsed.followUpTasks)
                    ? parsed.followUpTasks
                        ...
                        .slice(0, 5)
                    : null,
                suggestedSearchQuery: typeof parsed.suggestedSearchQuery === 'string' && parsed.suggestedSearchQuery.trim()
