...
import { XcomOAuthTokenModel } from '../models/XcomOAuthToken.js';
...
export class XcomService {
     ...
     */
    createStateToken(userId, codeVerifier, mode = 'link') {
        const state = {
            userId,
            codeVerifier,
            mode,
    ...
    async postTweet(userId, text) {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error?.message || errorData.detail || 'Failed to post tweet to X.com');
        }
