...
export class SlackActionsService {
     ...
     */
    async extractToItem(workspaceId, channelId, messageTs, userId, options = {}) {
        // Find the message
        ...
        if (slackMessage.item_id && !options.force) {
            ...
            if (existingItem) {
                return {
                    ...
                    itemType: existingItem.type || 'note',
        ...
        let itemType = options.type || 'note';
        if (!options.type) {
            ...
            try {
                ...
                if (classification.tags.some((tag) => ['task', 'todo', 'action', 'reminder'].includes(tag.toLowerCase())) ||
     ...
     */
    async extractToTask(workspaceId, channelId, messageTs, userId) {
