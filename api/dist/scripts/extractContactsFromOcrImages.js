...
function resolveBatchSize() {
    ...
    if (Number.isNaN(parsed) || parsed <= 0) {
        console.warn(`[ContactsScript] Invalid CONTACT_EXTRACTION_BATCH_SIZE="${raw}". Falling back to ${DEFAULT_BATCH_SIZE}.`);
        return DEFAULT_BATCH_SIZE;
    }
    return parsed;
}
...
const enforceImageFilter = process.env.CONTACT_EXTRACTION_REQUIRE_IMAGE_FILTER !== 'false';
const scriptOptions = {
    ownerId: resolvedOwnerId,
    itemId: resolvedItemId,
    batchSize: resolveBatchSize(),
    skipExisting: !forceProcessing &&
        !resolvedItemId &&
...
function buildAggregateKey(_name, email) {
...
function isImageAttachment(attachment) {
    ...
    const candidate = (attachment.originalname ?? attachment.filename ?? '').toLowerCase();
...
function addCandidatesFromText(collection, text, context) {
...
async function processAttachment(item, attachment, attachmentIndex, aggregation) {
    ...
    try {
        ...
        console.error(`[ContactsScript] Failed to parse attachment "${attachment.filename}" for item ${item.id}:`, error);
        return 0;
    }
}
...
function buildFetchQuery(cursor, limit) {
    if (scriptOptions.itemId) {
        const values = [scriptOptions.itemId];
        let text = `
    ...
    const values = [];
    const conditions = ['deleted_at IS NULL', 'attachments IS NOT NULL'];
    ...
    if (cursor) {
        ...
        conditions.push(`(created_at, id) > ($${createdAtIndex}::timestamptz, $${idIndex}::uuid)`);
    }
    values.push(limit);
...
async function fetchNextBatch(cursor, limit) {
    ...
    const createdAt = lastRow.created_at instanceof Date
...
async function main() {
    ...
    try {
        while (true) {
            const { rows, nextCursor } = await fetchNextBatch(cursor, scriptOptions.batchSize);
            if (!rows.length) {
                break;
            }
            stats.batches += 1;
            console.log(`[ContactsScript] Processing batch ${stats.batches} (${rows.length} items)`);
            for (const item of rows) {
                ...
                try {
                    ...
                    stats.itemsFailed += 1;
                    console.error(`[ContactsScript] Failed to process item ${item.id}:`, error);
                }
            }
            if (scriptOptions.itemId) {
                break;
...
void main();
//# sourceMappingURL=extractContactsFromOcrImages.js.map