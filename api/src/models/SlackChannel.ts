...
export class SlackChannelModel {
  static async findByChannelId(workspaceId: string, channelId: string): Promise<SlackChannel | null> {
    const result = await pool.query(
      'SELECT * FROM slack_channels WHERE workspace_id = $1 AND channel_id = $2',
      [workspaceId, channelId]
    );
    return result.rows[0] || null;
  }

  static async findByWorkspaceId(workspaceId: string): Promise<SlackChannel[]> {
    const result = await pool.query(
      'SELECT * FROM slack_channels WHERE workspace_id = $1 ORDER BY channel_name ASC',
      [workspaceId]
    );
    return result.rows;
  }

  static async findById(id: string): Promise<SlackChannel | null> {
    const result = await pool.query(
      'SELECT * FROM slack_channels WHERE id = $1',
      [id]
    );
    return result.rows[0] || null;
  }

  static async create(input: CreateSlackChannelInput): Promise<SlackChannel> {
    const result = await pool.query(
  ...
  static async update(workspaceId: string, channelId: string, input: UpdateSlackChannelInput): Promise<SlackChannel> {
    const fields: string[] = [];
  ...
  static async createOrUpdate(input: CreateSlackChannelInput): Promise<SlackChannel> {
  ...
  static async delete(workspaceId: string, channelId: string): Promise<boolean> {
    const result = await pool.query(
      'DELETE FROM slack_channels WHERE workspace_id = $1 AND channel_id = $2 RETURNING id',
      [workspaceId, channelId]
    );
