...
export class CollectionItemModel {
  static async addItem(
    ...
    return result.rows[0] as CollectionItem;
  }
  ...
  static async countByCollection(collectionId: string): Promise<number> {
    const result = await pool.query(
  ...
  static async findByItem(itemId: string): Promise<Collection[]> {
    const result = await pool.query(
      `SELECT c.*
       FROM collections c
  ...
  static async isItemInCollection(
    collectionId: string,
    itemId: string
  ): Promise<boolean> {
    const result = await pool.query(
