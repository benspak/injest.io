...
export class LoginSessionModel {
  static async create(userId: string, loginAt?: Date): Promise<LoginSession> {
    const loginTimestamp = loginAt || new Date();
    const result = await pool.query(
      'INSERT INTO login_sessions (user_id, login_at) VALUES ($1, $2) RETURNING *',
      [userId, loginTimestamp]
    );
  ...
  static async findByUserId(
    ...
    const result = await pool.query(
      'SELECT * FROM login_sessions WHERE user_id = $1 ORDER BY login_at DESC LIMIT $2 OFFSET $3',
      [userId, limit, offset]
    );
    return result.rows;
  }
  ...
  static async countByUserId(userId: string): Promise<number> {
    const result = await pool.query(
      'SELECT COUNT(*) as count FROM login_sessions WHERE user_id = $1',
      [userId]
    );
    return parseInt(result.rows[0].count, 10);
  }

  static async getLoginStreak(userId: string): Promise<{
    ...
    const result = await pool.query(
      `SELECT DISTINCT DATE(login_at AT TIME ZONE 'UTC') as login_date
       FROM login_sessions
       WHERE user_id = $1
         AND login_at >= $2
         AND login_at <= $3
