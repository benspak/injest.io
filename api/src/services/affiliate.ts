...
import { ReferralCommissionModel, type CommissionStatus } from '../models/ReferralCommission.js';
...
export class AffiliateService {
   ...
   */
  static async transferCommission(
    ...
    try {
      ...
      await ReferralCommissionModel.updateStatus(
        commissionId,
        'paid',
        transfer.id,
        new Date()
   ...
   */
  static async retryCommission(commissionId: string): Promise<void> {
    ...
    await this.transferCommission(
      referrer.stripe_connect_account_id,
      comm.amount_cents,
      comm.id
    );
  }
