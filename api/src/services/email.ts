...
export class EmailService {
   ...
   */
  private buildFromAddress(email: string, senderName?: string | null): string {
...
The Injest.io Team
  ...
  async sendItemShareEmail(params: {
    ...
    const formatSection = (label: string, value?: string | null): string => {
    ...
    let emailAttachments: Array<{ filename: string; content: string; contentType: string }> | undefined;
    ...
    if (Array.isArray(item.attachments) && item.attachments.length > 0) {
      ...
      const preparedAttachments: Array<{ filename: string; content: string; contentType: string }> = [];
    ...
    const defaultFrom =
      process.env.ITEM_SHARE_FROM_EMAIL || 'Injest <noreply@injest.io>';
    const fromAddress =
  ...
  async sendComposedEmail(params: {
    ...
    const defaultFrom =
      process.env.OUTBOUND_SEND_EMAIL || 'Injest <noreply@injest.io>';
    const fromAddress =
      fromEmail && fromEmail.trim().length > 0
  ...
  async getEmailAttachment(emailId: string, attachmentId: string, retries: number = 2): Promise<Blob> {
