 ...
 */
const normalizeNameForMatching = (name: string | null | undefined): string => {
  if (!name) {
    return '';
  }
 ...
 */
const buildUserFullName = (user: User): string | null => {
...
export class ContactEnrichmentService {
   ...
   */
  private async findPublicUsersByName(name: string): Promise<User[]> {
   ...
   */
  async matchContactToUser(contact: Contact): Promise<User | null> {
