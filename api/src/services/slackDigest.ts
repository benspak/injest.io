...
export class SlackDigestService {
   ...
   */
  async generateDigest(userId: string, workspaceId: string, date: Date = new Date()): Promise<UserDigest> {
    ...
    const digestChannels: DigestChannel[] = [];
    let totalMessages = 0;
    ...
    for (const channel of channels) {
      ...
      try {
        const summaryResult = await slackSummarizationService.summarizeChannelSegment(
          channel.channel_id,
          workspaceId,
          userId,
          {
   ...
   */
  async sendDigest(userId: string, workspaceId: string, digest: UserDigest): Promise<void> {
   ...
   */
  async generateAndSendDigestsForAllUsers(date: Date = new Date()): Promise<void> {
