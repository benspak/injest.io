 ...
 */
function formatSearchResults(results: Awaited<ReturnType<typeof slackSearchService.searchMessages>>): SlackBlock[] {
  if (results.length === 0) {
    return [
      {
        type: 'section',
 ...
 */
function formatSummary(summary: string, messageCount: number): SlackBlock[] {
  return [
    {
      type: 'header',
      text: {
...
export class SlackCommandsService {
   ...
   */
  async handleCommand(payload: SlackCommandPayload): Promise<SlackMessageResponse> {
  ...
  private async handleSummarizeCommand(
    ...
    try {
      ...
      if (args) {
        ...
        const result = await slackSummarizationService.summarizeChannelSegment(
          channelId,
          workspaceId,
          userId,
          {
   ...
   */
  private handleHelpCommand(): SlackMessageResponse {
    return {
      response_type: 'ephemeral',
      blocks: [
        {
