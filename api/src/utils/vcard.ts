...
const parsePropertyValue = (rawValue: string, params: string[]): string => {
  ...
  if (charsetParam) {
    try {
      ...
      if (charset && typeof Buffer !== 'undefined') {
        value = Buffer.from(value, 'binary').toString(charset as BufferEncoding);
      }
    } catch {
...
export const parseVCard = (content: string): ParsedVCardEntry[] => {
  if (!content) {
    return [];
  }

  const lines = unfoldLines(content.split(LINE_BREAK_REGEX));
  const entries: ParsedVCardEntry[] = [];

  let currentRaw: Record<string, string[]> | null = null;
