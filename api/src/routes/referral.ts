 ...
 * Body: { code: string }
 */
router.post('/code', async (req: AuthRequest, res: express.Response) => {
  try {
    if (!req.user) {
 ...
 */
router.get('/code', async (req: AuthRequest, res: express.Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
 ...
 */
router.get('/commissions', async (req: AuthRequest, res: express.Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    const commissions = await ReferralCommissionModel.getCommissionsByReferrerId(
      req.user.id,
      limit,
      offset
    );
 ...
 */
router.post('/connect/setup', async (req: AuthRequest, res: express.Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
    ...
    const isPlatformOnboardingError =
 ...
 */
router.get('/connect/status', async (req: AuthRequest, res: express.Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
