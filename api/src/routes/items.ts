...
import type { ParsedFileContent } from '../services/fileParser.js';
...
const formatPlanPriceWithInterval = (plan: SubscriptionPlan): string => {
...
const extractContactCandidatesFromText = (text?: string | null): ContactCandidate[] => {
...
async function persistContactCandidatesForItem(
  client: PoolClient,
  ownerId: string,
  itemId: string,
...
async function checkItemCreationLimit(
  ...
  if (itemCount >= limit) {
    ...
    const upgradeMessage = upgradePlan
      ? ` Upgrade to the ${upgradePlan.name} plan (${formatPlanPriceWithInterval(upgradePlan)}) for ${
          typeof upgradePlan.maxIndexedItems === 'number'
...
interface QueuedUploadJob {
  files: UploadedFileWithChecksum[];
  userId: string;
  title?: string;
  description?: string;
  url?: string;
...
interface UploadAcknowledgementFile {
  filename: string;
...
function buildTagList(input?: string | string[]): string[] | undefined {
  if (!input) {
    return undefined;
  }
...
async function deleteFileSafe(filename: string, logger?: (message: string) => void): Promise<void> {
  try {
    await fileStorageService.deleteFile(filename);
  } catch (cleanupError) {
    const msg = `[UPLOAD] Failed to delete file ${filename} during cleanup: ${(cleanupError as Error)?.message ?? cleanupError}`;
    if (logger) {
      logger(msg);
    } else {
      console.warn(msg);
...
async function processSingleFile(
  ...
  const log = (message: string) => console.log(`${stagePrefix} ${message}`);
  const warn = (message: string) => console.warn(`${stagePrefix} ${message}`);
  ...
  try {
    ...
    let finalTitle: string | undefined = providedTitle || undefined;
    let finalDescription: string | undefined = providedDescription || undefined;
    ...
    log(
      `Stage 2/4 complete (title: "${finalTitle}"${finalDescription ? ', description generated' : ''})`
    );
    ...
    if (!parsedTags) {
      try {
        ...
        warn(`Error auto-generating tags: ${(error as Error)?.message ?? error}`);
      }
    }
    ...
    const message =
      error?.code === '23505' && error?.duplicate_checksum
...
function scheduleUploadProcessing(job: QueuedUploadJob, loggerPrefix = '[UPLOAD]') {
  setImmediate(() => {
...
async function processQueuedUpload(job: QueuedUploadJob, loggerPrefix = '[UPLOAD]'): Promise<void> {
...
export async function handleCreateItem(req: AuthRequest, res: express.Response) {
  try {
    ...
    const files: UploadedFileWithChecksum[] = await Promise.all(
    ...
    if (files.length > 0) {
      ...
      const duplicateFiles: Array<{ checksum: string; filename: string; itemId: string; title?: string | null }> = [];
      ...
      if (acceptedFiles.length === 0) {
        const acknowledgement: UploadAcknowledgement = {
          queued: true,
          uploadedCount: 0,
    ...
    if (files.length > 0) {
      attachments = files.map((file: UploadedFileWithChecksum) => ({
        filename: file.filename,
        originalname: file.originalname,
        mimetype: file.mimetype,
        size: file.size,
    ...
    let parsedContentForContacts: ParsedFileContent | null = null;
    let contactCandidates: ContactCandidate[] = [];
    let contactExtractionSource: string | undefined;
    let contactTextSample: string | undefined;
    let contactFilename: string | undefined;
...
router.post('/:id/post-to-profile', async (req: AuthRequest, res: express.Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
...
router.delete('/:id/post-to-profile', async (req: AuthRequest, res: express.Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }
