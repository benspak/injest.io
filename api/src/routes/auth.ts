...
router.post('/forgot-password', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const resetToken = jwt.sign(
      { userId: user.id, email: user.email, passwordReset: true },
      JWT_SECRET,
      { expiresIn: PASSWORD_RESET_EXPIRATION } as SignOptions
    );
...
router.post('/reset-password', async (req: express.Request, res: express.Response) => {
  try {
    ...
    // Verify token
    let decoded: { userId: string; email: string; passwordReset?: boolean };
    try {
...
router.put('/inbound-handle', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ error: 'Unauthorized' });
    }

    const { handle } = req.body as { handle?: string | null };
