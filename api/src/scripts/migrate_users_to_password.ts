 ...
 */
async function sendMigrationEmail(email: string, tempPassword: string): Promise<void> {
 ...
 */
async function migrateUser(userId: string, email: string): Promise<void> {
...
async function main(): Promise<void> {
  try {
    ...
    const result = await pool.query(
      'SELECT id, email FROM users WHERE password_hash IS NULL ORDER BY created_at'
    );
