...
interface MigrationStats {
  oauthTokens: { xcom: number; slack: number };
  users: number;
  contacts: number;
  items: number;
 ...
 */
async function encryptOAuthTokens(): Promise<{ xcom: number; slack: number; errors: Array<{ table: string; id: string; error: string }> }> {
    ...
    for (const token of xcomResult.rows) {
      try {
        ...
        if (token.access_token && !token.access_token.includes(':')) {
          ...
          await pool.query(
            'UPDATE xcom_oauth_tokens SET access_token = $1, refresh_token = $2 WHERE id = $3',
            [encryptedAccess, encryptedRefresh, token.id]
          );
          xcomCount++;
        }
      } catch (error) {
    ...
    for (const token of slackResult.rows) {
      try {
        ...
        if (token.access_token && !token.access_token.includes(':')) {
          ...
          await pool.query(
            'UPDATE slack_oauth_tokens SET access_token = $1, authed_user_token = $2 WHERE id = $3',
            [encryptedAccess, encryptedAuthed, token.id]
          );
          slackCount++;
        }
      } catch (error) {
 ...
 */
async function encryptUsers(): Promise<{ count: number; errors: Array<{ table: string; id: string; error: string }> }> {
  const errors: Array<{ table: string; id: string; error: string }> = [];
  let count = 0;
 ...
 */
async function encryptContacts(): Promise<{ count: number; errors: Array<{ table: string; id: string; error: string }> }> {
  const errors: Array<{ table: string; id: string; error: string }> = [];
  let count = 0;
 ...
 */
async function encryptItems(): Promise<{ count: number; errors: Array<{ table: string; id: string; error: string }> }> {
  const errors: Array<{ table: string; id: string; error: string }> = [];
  let count = 0;
 ...
 */
async function encryptSearchDocuments(): Promise<{ count: number; errors: Array<{ table: string; id: string; error: string }> }> {
  const errors: Array<{ table: string; id: string; error: string }> = [];
  let count = 0;
    ...
    for (const doc of result.rows) {
      try {
        ...
        if (updates.length > 0) {
          values.push(doc.id);
          await pool.query(
            `UPDATE search_documents SET ${updates.join(', ')} WHERE id = $${paramCount}`,
            values
          );
          count++;
        }
