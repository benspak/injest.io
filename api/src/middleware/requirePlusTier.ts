...
export async function requirePlusTier(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (bearerToken) {
      try {
        const decoded = jwt.verify(bearerToken, JWT_SECRET) as { userId: string; email: string };
