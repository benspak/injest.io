...
import { Search, Plus, CheckCircle2, Circle, Brain, Sparkles } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const loadItems = async () => {
    try {
      ...
      setItems(data.filter(item => !item.isTask));
    } catch (error) {
  ...
  const loadTasks = async () => {
    try {
      const data = await api.getItems({ isTask: true, limit: 50 });
      setTasks(data);
    } catch (error) {
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const result = await api.generate(generatePrompt);
      ...
      <div className="container mx-auto p-4">
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>Generate drafts, summaries, or replies using your stored context</CardDescription>
          </CardHeader>
          <CardContent>
            <Input
              placeholder="e.g., Draft an email about the meeting notes"
        ...
        {tasks.length > 0 && (
          <div className="mb-8">
            ...
            <div className="space-y-2">
              {tasks.map((task) => (
                <Card key={task.id} className={task.taskCompleted ? 'opacity-60' : ''}>
                  <CardContent className="p-4">
                    <div className="flex items-start gap-3">
                      <button
                        onClick={() => handleToggleTask(task.id, task.taskCompleted || false)}
                        className="mt-1"
                      >
                        {task.taskCompleted ? (
                      ...
                      <div className="flex-1">
                        <h3 className="font-medium">{task.title || task.raw.substring(0, 60)}</h3>
                        {task.clean && (
                          <p className="text-sm text-muted-foreground mt-1">{task.clean}</p>
                        )}
                      </div>
                    </div>
                  </CardContent>
