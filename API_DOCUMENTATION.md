...
External (Plus-tier API key): https://api.injest.io/api/external | http://localhost:5555/api/external
...
- **External API keys** generated by Plus-tier users for `/api/external` integrations. Send in the `x-api-key: <key>` header. Session tokens are also accepted on `/api/external` for development convenience.
...
All routes require an authenticated session and a Plus (or above) subscription tier. The plaintext API key is only returned in the response to `POST /api/auth/api-key`.
